...
func (l *AddLogic) Add(req *types.AddUserEducationReq) (resp *types.IdResp, err error) {
	userId := ctxdata.GetUserIdFromCtx(l.ctx)

	var val model.AppUserEducation
	...
	if val.Id > 0 {
		err = l.svcCtx.AppUserEducationModel.Update(l.ctx, &val)
		if err != nil {
	...
	res, err := l.svcCtx.AppUserEducationModel.Insert(l.ctx, &val)
	if err != nil {
		return &types.IdResp{Id: 0}, nil
	}
