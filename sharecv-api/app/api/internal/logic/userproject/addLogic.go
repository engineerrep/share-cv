...
func (l *AddLogic) Add(req *types.AddUserProjectReq) (resp *types.IdResp, err error) {
	userId := ctxdata.GetUserIdFromCtx(l.ctx)

	var val model.AppUserProject
	...
	if val.Id > 0 {
		err = l.svcCtx.AppUserProjectModel.Update(l.ctx, &val)
		if err != nil {
	...
	res, err := l.svcCtx.AppUserProjectModel.Insert(l.ctx, &val)
	if err != nil {
		return &types.IdResp{Id: 0}, nil
	}
