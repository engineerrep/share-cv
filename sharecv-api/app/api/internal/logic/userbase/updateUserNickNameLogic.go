...
func (l *UpdateUserNickNameLogic) UpdateUserNickName(req *types.UpdateUserNickNameReq) (resp *types.StatusResp, err error) {
	userID := ctxdata.GetUserIdFromCtx(l.ctx)
	userInfo, err := l.svcCtx.AppUserInfoModel.FindOne(l.ctx, userID)
	if userInfo == nil {
		userInfo = &model.AppUserInfo{UserId: userID, Nickname: req.Nickname}
