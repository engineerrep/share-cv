...
func (l *AddLogic) Add(req *types.AddUserVideoReq) (resp *types.StatusResp, err error) {
	...
	if len(list) >= l.svcCtx.Config.User.MaxVideo {
		return nil, errorx.NewCodeError(errorx.MaxCountErrorCode)
	}

	var entitys []*model.AppUserVideo
	for _, item := range req.List {
		var val model.AppUserVideo
	...
	_, err = l.svcCtx.AppUserVideoModel.InsertByList(l.ctx, entitys)
	if err != nil {
