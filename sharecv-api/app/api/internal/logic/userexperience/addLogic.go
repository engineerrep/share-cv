...
func (l *AddLogic) Add(req *types.AddUserExperienceReq) (resp *types.IdResp, err error) {
	userId := ctxdata.GetUserIdFromCtx(l.ctx)

	var val model.AppUserExperience
	...
	if val.Id > 0 {
		err = l.svcCtx.AppUserExperienceModel.Update(l.ctx, &val)
		if err != nil {
	...
	res, err := l.svcCtx.AppUserExperienceModel.Insert(l.ctx, &val)
	if err != nil {
		return &types.IdResp{Id: 0}, nil
	}
