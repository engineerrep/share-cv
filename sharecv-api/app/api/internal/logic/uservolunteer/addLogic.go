...
func (l *AddLogic) Add(req *types.ChangeUserVolunteerReq) (resp *types.IdResp, err error) {
	userId := ctxdata.GetUserIdFromCtx(l.ctx)

	var val model.AppUserVolunteer
	...
	if val.Id > 0 {
		err = l.svcCtx.AppUserVolunteerModel.Update(l.ctx, &val)
		if err != nil {
	...
	res, err := l.svcCtx.AppUserVolunteerModel.Insert(l.ctx, &val)
	if err != nil {
		return &types.IdResp{Id: 0}, nil
	}
